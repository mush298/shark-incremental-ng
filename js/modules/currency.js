const CURRENCIES = {
    fish: {
        ...
        get gain() {
            ...
            let us = E(1).add(player.universe.size.log10().add(40))

            if (hasUMilestone(1)) x = expPow(x,us)
            if (hasRMilestone(6)) x = expPow(x, '69420')
            ...
            x = x.overflow('ee9e15', 0.25)


            let hc = E('5e22')
            hc = hc.mul(player.universe.rocket.explored.log10().pow(2))
            hc = hc.pow10().pow10()

            if (!hasRMilestone(0)) hc = E('ee5e22')

            hasUMilestone(8) ? x = x.overflow(hc, 0.1) : x = x.min(hc)

            
            let u = E(9e15).pow(E(1).add(player.universe.amount.div(2))).pow10().pow10()
    ...
    'dark-matter': {
        ...
        get gain() {
            ...
            if (hasUMilestone(6)) x = expPow(x, 1.25)

            if (hasRMilestone(5)) x = expPow(x, 1.5)

                x = expPow(x, constellationBoostEffect(3,false))
    ...
    universe_size: {
        get amount() { return player.universe.size },
        set amount(v) { player.universe.size  = v.max(0) },
        ...
        get mult() { 
            ...
            if (hasSMilestone(1)) pow = pow.mul(2)

            if (hasSMilestone(5)) pow = pow.mul(2)
            
            return player.universe.antimatter.pow(pow).add(1) },

        get gain() {
            let x = E(1e-38)
            x = x.mul(CURRENCIES.universe_size.mult)
            return x
        },
    
        get passive() { return player.universe.amount.gte(1)},
    },

    universe: {
        next(s=player.universe.amount) {
            let x = E(9e15).pow(E(1).add(s.div(2)))
            return x.pow10().pow10()
        },
        get require() { return this.next() },

        get moreArg() { return [this.next(player.universe.amount.add(tmp.currency_gain.universe))] },
        ...
        get amount() { return player.universe.amount },
        set amount(v) { player.universe.amount = v.max(0) },

        get gain() {
            let x = player.fish, u = player.universe.amount

            if (x.lt("ee9e15")) return E(0)

            x = E(1)
    
            return x.max(0).floor()
        },

        get passive() { return 0 },
    }, 
    antimatter: {
        get amount() { return player.universe.antimatter },
        set amount(v) { player.universe.antimatter  = v.max(0) },

        get gain() {
            let x = E(1)

             if (player.humanoid.forge.level.antimatter >= 6) {
                x = x.mul('1e100')
                x = x.pow(forgeUpgradeEffect('antimatter'))
             } else {
                x = x.mul(forgeUpgradeEffect('antimatter'))
             }

            if (hasSMilestone(3)) x = x.pow(2)

            if (hasRMilestone(3)) x = x.pow(player.universe.rocket.explored.log10().pow(0.25).add(1))

            return x
        },

        get passive() { return hasUMilestone(0) },
    },
    explored: {
        get amount() { return player.universe.rocket.explored },
        set amount(v) { player.universe.rocket.explored  = v.max(0) },

        get gain() {

            let t = player.universe.rocket.tier

            let e = player.universe.rocket.explored

            

            let x = E(1)

            x = x.mul(t.pow(2).pow10())

            if (hasRMilestone(1)) x = x.mul(e.log10().pow(2.5))
            ...
            if (x.gte(c)) {
                x = x.overflow(c, 0.001)
            }

            return x
        },

        get passive() { return player.universe.rocket.tier.gte(1) },
    },
}
    ...
    for (let i = 0; i < CONSTELLATION.boosts.length; i++) {
        ...
        CURRENCIES[b.name] = {
            ...
            get gain() {
                ...
                if (hasUMilestone(3)) x = x.pow(1.25)
                return x
            },
        }
    }
