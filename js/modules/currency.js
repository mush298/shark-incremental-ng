const CURRENCIES = {
    fish: {
        ...
        get gain() {
            ...
            if (hasSMilestone(12)) x = x.pow(dilationEffect(0))
    ...
    humanoid: {
        ...
        get mult() { return getPAEffect(3).pow(dilationEffect(4))},
    ...
    remnants: {
        ...
        get gain() {
            ...
            if (hasSMilestone(13)) x = x.pow(dilationEffect(1))
    ...
    'dark-matter': {
        ...
        get gain() {
            ...
            x = x.pow(dilationEffect(6))
    
            return x.floor()
        },

        get passive() { return hasSMilestone(16) ? 1 : 0 },
    },
    'dilated': {
        get require() { return E('ee72500') },

        get amount() { return player.singularity.dilated },
        set amount(v) { player.singularity.dilated = v.max(0) },

        get total() { return player.singularity.total_dilated },
        set total(v) { player.singularity.total_dilated = v.max(0) },

        get gain() {
            let x = player.fish.log10().log10().div(72500)
            x = x.mul(remnantUpgEffect(11))
            x = x.mul(getSharkRankBonus('dilated'))
            x = x.pow(dilationEffect(5))
            if (hasSMilestone(16)) x = x.pow(player.singularity.dm.log10().div(100).mul(2).add(1))
            x = x.pow(sharkUpgEffect('d1'))
            if (x.gte(1e100)) {
                x = E(100).add(x.log10().pow(0.5)).pow10()
            }

            let softcap2 = E('1e1000')
            if (hasResearch('dm8')) softcap2 = E('e2500').pow(getPAEffect(6).add(1))

            if (x.gte(softcap2)) {
                x = E(softcap2.log10()).add(x.log10().pow(0.25)).pow10()
            }

            if (x.gte('ee10000')) {
                x = E(10000).add(x.log10().pow(0.1)).pow10()
            }
    ...
    'dark-faith': {
        next(s=player.singularity.dark_faith) { return Decimal.pow(10,E(14).add(s)) },
        get require() { return this.next() },

        get amount() { return player.singularity.dark_faith },
        set amount(v) { player.singularity.dark_faith = v.max(0) },

        get total() { return player.singularity.tdf },
        set total(v) { player.singularity.tdf = v.max(0) },

        get gain() {
            let f = tmp.total_faith
            let x = f.log10().floor().sub(13).sub(player.singularity.dark_faith)
            return x
        },
