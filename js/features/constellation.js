const CONSTELLATION = {
    get exp() {
        ...
        x = x.mul(hasRMilestone(7) ? 5 : 1)
        return x
    },

    get base() {
        let x = player.fish.add(10).iteratedlog(10,2).mul(CURRENCIES.remnants.amount.add(1).log10()).mul(CURRENCIES["dark-matter"].amount.add(1).log10()).pow(hasRMilestone(2) ? 1.5 : 1)
    ...
    boosts: [
        {
            ...
            req: 50,
            effect(r,d) {
                let x = d ? r.log10().log10().add(1) : r.log10().pow(0.25).div(2).add(1)
                return x
            },
            effDesc: x=>formatMult(x,3),
        },
