const FORGE = {
    ...
    antimatter: {
        ...
        cost: [
            [
                ['antimatter',1000, true],
                ['sharkium', 1e9],
            ],[
                ['antimatter',100000, true]
            ],[
                ['antimatter',250e9, true]
            ],[
                ['fish','ee2e21', true]
            ],[
                ['sharkium','1e30', true],
        ...
        effect(l) {
            ...
            if (E(l).gte(2)) x = x.pow(1.5)

            if (E(l).gte(5)) x = x.pow(E(l).pow(2))

            if (E(l).gte(6)) x = x.log10().div(100)

            if (E(l).gte(8)) x = x.pow(l).pow(0.25)

            return x
        },
        effDesc: x => player.humanoid.forge.level.antimatter >= 6 ? formatPow(x) : formatMult(x),
    }
}
