const CURRENCIES = {
    fish: {
        ...
        get gain() {
            ...
            x = x.pow(sharkUpgEffect('s4')).pow(tmp.explore_eff[2]).pow(coreReactorEffect(0)).pow(getSharkRankBonus('fish')).pow(simpleETEffect(12)).pow(BHEffect(0))
    ...
    prestige: {
        ...
        get gain() {
            ...
            x = x.pow(tmp.explore_eff[3]).pow(simpleETEffect(13)).pow(getSharkRankBonus('prestige')).pow(forgeUpgradeEffect('shard')).pow(BHEffect(1))
    ...
    core: {
        ...
        get gain() {
            ...
            x = x.mul(getSharkBonus("core")).mul(getCRBoost(1)).pow(simpleETEffect(14)).pow(BHEffect(2))
    ...
    humanoid: {
        ...
        get mult() { return getPAEffect(3).mul(BHEffect(3)) },
    ...
    black_hole: {
        get require() { return E('e2.5e6') },

        get amount() { return player.black_hole.shards },
        set amount(v) { player.black_hole.shards = v.max(0) },

        get total() { return player.black_hole.total },
        set total(v) { player.black_hole.total = v.max(0) },

        get gain() {
            let x = player.core.total.log10().div(2.5e6)
            x = x.mul(BHEffect(5))
    ...
    bh_size: {
        ...
        get amount() { return player.black_hole.size },
        set amount(v) { player.black_hole.size = v.max(0) },

        get gain() {
            let x = E(0).add(player.black_hole.shards)
            x = x.mul(sharkUpgEffect('b1'))
